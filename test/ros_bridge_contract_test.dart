...
import 'package:marcotest/services/ros_bridge_client.dart';
import 'package:marcotest/services/ros_connection_controller.dart';

void main() {
  TestWidgetsFlutterBinding.ensureInitialized();
  test('adres ws://ROBOT_IP:9090 bicimine normalize edilir', () {
    expect(
      RosBridgeClient.normalizeAddress('localhost').toString(),
      'ws://localhost:9090/',
    );
    expect(
      RosBridgeClient.normalizeAddress('ws://192.168.1.20:9090').port,
      9090,
    );
    expect(
      () => RosBridgeClient.normalizeAddress('ftp://robot'),
  ...
  test('kopunca yeniden baglanir ve manuel dead-man sifir yollar', () async {
    ...
    while (!client.state.value.isConnected &&
  ...
  test(
    'Phase 10 mock_plc servisleri rosbridge uzerinden calisir',
    () async {
      final ros = RosConnectionController.instance..init();
      final client = ros.client;
      ...
      await ros.connect(liveUrl!);
      final status = await firstStatus.future.timeout(
        const Duration(seconds: 5),
      );
      expect(status, contains('mission_state'));
      expect((await ros.resetMissionSafety())['accepted'], isTrue);
      final manual = await ros.submitManualTask(
        taskId: 'mobile_integration_test',
        pickupNode: 'alma_1',
        dropoffNode: 'birak_1',
      );
      expect(manual['accepted'], isTrue);
      expect((await ros.cancelMission())['accepted'], isTrue);
      await Future<void>.delayed(const Duration(milliseconds: 500));
      expect((await ros.resetMissionSafety())['accepted'], isTrue);
      expect((await ros.startMission())['accepted'], isTrue);
      expect((await ros.cancelMission())['accepted'], isTrue);
